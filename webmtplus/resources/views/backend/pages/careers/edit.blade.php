...
@props(['pageTitle' => __('admin.careers.edit_career')])
@section('content-backend')
    ...
    <form action="{{ route('admin.careers.update', $career->slug) }}" method="POST" enctype="multipart/form-data" id="careerForm">
        @csrf
        @method('PUT')
        ...
        <div class="row">
                ...
                <div class="card bg-white rounded-10 border border-white p-20 mb-4">
                    ...
                    <div class="tab-content" id="basicInfoTabContent">
                        ...
                        <div class="tab-pane fade show active" id="vi-basic" role="tabpanel">
                            <div class="mb-3">
                                ...
                                <input type="text" name="title_vi" class="form-control @error('title_vi') is-invalid @enderror"
                                       value="{{ old('title_vi', $career->title_vi) }}" required>
                                @error('title_vi')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description_vi" rows="4" class="form-control @error('description_vi') is-invalid @enderror">{{ old('description_vi', $career->description_vi) }}</textarea>
                        ...
                        <div class="tab-pane fade" id="en-basic" role="tabpanel">
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-semibold">{{ __('admin.careers.description') }}</label>
                                <textarea name="description_en" rows="4" class="form-control @error('description_en') is-invalid @enderror">{{ old('description_en', $career->description_en) }}</textarea>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="job_type" class="form-select form-control @error('job_type') is-invalid @enderror" required>
                                <option value="full-time" {{ old('job_type', $career->job_type) === 'full-time' ? 'selected' : '' }}>{{ __('admin.careers.full_time') }}</option>
                                <option value="part-time" {{ old('job_type', $career->job_type) === 'part-time' ? 'selected' : '' }}>{{ __('admin.careers.part_time') }}</option>
                                <option value="contract" {{ old('job_type', $career->job_type) === 'contract' ? 'selected' : '' }}>{{ __('admin.careers.contract') }}</option>
                                <option value="internship" {{ old('job_type', $career->job_type) === 'internship' ? 'selected' : '' }}>{{ __('admin.careers.internship') }}</option>
                            </select>
                            @error('job_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="salary_currency" class="form-select form-control @error('salary_currency') is-invalid @enderror">
                                <option value="VND" {{ old('salary_currency', $career->salary_currency) === 'VND' ? 'selected' : '' }}>VND</option>
                                <option value="USD" {{ old('salary_currency', $career->salary_currency) === 'USD' ? 'selected' : '' }}>USD</option>
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="salary_period" class="form-select form-control @error('salary_period') is-invalid @enderror">
                            <option value="monthly" {{ old('salary_period', $career->salary_period) === 'monthly' ? 'selected' : '' }}>{{ __('admin.careers.monthly') }}</option>
                            <option value="annually" {{ old('salary_period', $career->salary_period) === 'annually' ? 'selected' : '' }}>{{ __('admin.careers.annually') }}</option>
                            <option value="hourly" {{ old('salary_period', $career->salary_period) === 'hourly' ? 'selected' : '' }}>{{ __('admin.careers.hourly') }}</option>
                        </select>
                        @error('salary_period')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="card bg-white rounded-10 border border-white p-20 mb-4">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="email" name="application_email" class="form-control @error('application_email') is-invalid @enderror"
                                   value="{{ old('application_email', $career->application_email) }}" placeholder="hr@company.com">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" name="application_url" class="form-control @error('application_url') is-invalid @enderror"
                               value="{{ old('application_url', $career->application_url) }}" placeholder="https://...">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" min="1" name="positions_available" class="form-control @error('positions_available') is-invalid @enderror"
                               value="{{ old('positions_available', $career->positions_available) }}">
            ...
            <div class="col-lg-4">
                ...
                <div class="card bg-white rounded-10 border border-white p-20 mb-4">
                    ...
                    @if($career->image)
                        <div class="mb-3">
                            <label class="form-label fw-semibold">{{ __('admin.careers.current_image') }}</label>
                            <img src="{{ $career->image }}" class="img-fluid rounded" style="max-height: 200px;">
                        </div>
                    @endif
                ...
                <div class="card bg-white rounded-10 border border-white p-20 mb-4">
                    ...
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" id="is_featured" name="is_featured" {{ old('is_featured', $career->is_featured) ? 'checked' : '' }}>
                    ...
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" {{ old('is_active', $career->is_active) ? 'checked' : '' }}>
...
function addResponsibilityVI(value = '') {
    const list = document.getElementById('responsibilities-vi-list');
    const div = document.createElement('div');
    div.className = 'input-group mb-2';
    div.innerHTML = `
        <input type="text" name="responsibilities_vi[]" class="form-control" placeholder="{{ __('admin.careers.enter_responsibility') }}" value="${value}">
...
function addResponsibilityEN(value = '') {
    const list = document.getElementById('responsibilities-en-list');
    const div = document.createElement('div');
    div.className = 'input-group mb-2';
    div.innerHTML = `
        <input type="text" name="responsibilities_en[]" class="form-control" placeholder="{{ __('admin.careers.enter_responsibility') }}" value="${value}">
        <button type="button" class="btn btn-outline-danger" onclick="this.parentElement.remove()">
            <i class="ri-delete-bin-line"></i>
        </button>
    `;
...
function addQualificationVI(value = '') {
    const list = document.getElementById('qualifications-vi-list');
    const div = document.createElement('div');
    div.className = 'input-group mb-2';
    div.innerHTML = `
        <input type="text" name="qualifications_vi[]" class="form-control" placeholder="{{ __('admin.careers.enter_qualification') }}" value="${value}">
        <button type="button" class="btn btn-outline-danger" onclick="this.parentElement.remove()">
            <i class="ri-delete-bin-line"></i>
        </button>
    `;
...
function addQualificationEN(value = '') {
    const list = document.getElementById('qualifications-en-list');
    const div = document.createElement('div');
    div.className = 'input-group mb-2';
    div.innerHTML = `
        <input type="text" name="qualifications_en[]" class="form-control" placeholder="{{ __('admin.careers.enter_qualification') }}" value="${value}">
        <button type="button" class="btn btn-outline-danger" onclick="this.parentElement.remove()">
            <i class="ri-delete-bin-line"></i>
        </button>
    `;
...
function addBenefitVI(value = '') {
    const list = document.getElementById('benefits-vi-list');
    const div = document.createElement('div');
    div.className = 'input-group mb-2';
    div.innerHTML = `
        <input type="text" name="benefits_vi[]" class="form-control" placeholder="{{ __('admin.careers.enter_benefit') }}" value="${value}">
        <button type="button" class="btn btn-outline-danger" onclick="this.parentElement.remove()">
            <i class="ri-delete-bin-line"></i>
        </button>
    `;
...
function addBenefitEN(value = '') {
    const list = document.getElementById('benefits-en-list');
    const div = document.createElement('div');
    div.className = 'input-group mb-2';
    div.innerHTML = `
        <input type="text" name="benefits_en[]" class="form-control" placeholder="{{ __('admin.careers.enter_benefit') }}" value="${value}">
        <button type="button" class="btn btn-outline-danger" onclick="this.parentElement.remove()">
            <i class="ri-delete-bin-line"></i>
        </button>
    `;
...
document.addEventListener('DOMContentLoaded', function() {
    // Load existing responsibilities VI
    @if($career->responsibilities_vi)
        @foreach($career->responsibilities_vi as $resp)
            addResponsibilityVI('{{ addslashes($resp) }}');
        @endforeach
    @else
        addResponsibilityVI();
    @endif

    // Load existing responsibilities EN
    @if($career->responsibilities_en)
        @foreach($career->responsibilities_en as $resp)
            addResponsibilityEN('{{ addslashes($resp) }}');
        @endforeach
    @else
        addResponsibilityEN();
    @endif

    // Load existing qualifications VI
    @if($career->qualifications_vi)
        @foreach($career->qualifications_vi as $qual)
            addQualificationVI('{{ addslashes($qual) }}');
        @endforeach
    @else
        addQualificationVI();
    @endif

    // Load existing qualifications EN
    @if($career->qualifications_en)
        @foreach($career->qualifications_en as $qual)
            addQualificationEN('{{ addslashes($qual) }}');
        @endforeach
    @else
        addQualificationEN();
    @endif

    // Load existing benefits VI
    @if($career->benefits_vi)
        @foreach($career->benefits_vi as $ben)
            addBenefitVI('{{ addslashes($ben) }}');
        @endforeach
    @else
        addBenefitVI();
    @endif

    // Load existing benefits EN
    @if($career->benefits_en)
        @foreach($career->benefits_en as $ben)
            addBenefitEN('{{ addslashes($ben) }}');
        @endforeach
    @else
        addBenefitEN();
    @endif
